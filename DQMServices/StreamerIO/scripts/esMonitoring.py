...
class FDOutput(AsyncLineReaderTimeoutMixin, asyncore.file_dispatcher):
    def __init__(self, fd, es, close_socket=None):
    ...
    def handle_close(self):
        super(FDOutput, self).handle_close()
...
def launch_monitoring(args):
    ...
    log_handler = FDOutput(fd=p.stdout.fileno(), es=es, close_socket=json_handler)
