...
class Playback(Applet):
    ...
    def do_exec(self):
        ...
        while True:
            now = time.time()
            if (now - last_write) > self.lumi_duration:
                last_write = now
...
set -x #echo on
TODAY=$(date)
logname="{log_path}/hlt_run$4_pid$$.log"
exec 1>>$logname 2>&1

echo startDqmRun invoked $TODAY with arguments $1 $2 $3 $4 $5 $6 $7 $8
export SCRAM_ARCH=$2
cd $1
cd base
...
# but keep the  old one for debugging
GZIP_LOG="{log_path}hlt_run$4_pid$$.logz"
export GZIP_LOG

# for process substitution
set +o posix

# the stdout and stderr of *us* will be redirected into ztail.py;
# ztail.py will then become the child of esMonitoring.py (or cmssw)
exec 1> >(exec ztee.py -q $GZIP_LOG) 2>&1
...
class FrameworkJob(Applet):
    ...
    def _prepare_files(self):
        self.home_path = os.path.join(self.opts.work_control, "%s_%s" % (self.name, hex(id(self))))
        self.home_path = os.path.realpath(self.home_path)

        base = os.path.join(self.home_path, "base")
        os.makedirs(base)

        cmsset_file = os.path.join(base, "cmsset_default.sh")
        self.log.info("Creating: %s", cmsset_file)
        with open(cmsset_file, "w") as f:
            f.write("# empty \n")
        ...
        template = start_dqm_job_nolog
        body = template.format(log_path=self.log_path)
        f.write(body)
        f.close()
...
if __name__ == "__main__":
    ...
    subdirectories = ["ramdisk", "output", "control", "logs", "dqm_monitoring"]
    ...
    parser.add_argument("--work", "-w", type=str, help="Working directory (used for inputs,outputs,monitoring and logs).", default="/tmp/pplay/")
    ...
    playback_group = parser.add_argument_group('Playback', 'Playback parameters.')
    ...
    playback_group.add_argument("--nlumi", type=int, help="Number of lumis to deliver, -1 for forever.", default=-1)
    playback_group.add_argument("--run", type=int, help="Run number, -1 for autodiscovery.", default=-1)
