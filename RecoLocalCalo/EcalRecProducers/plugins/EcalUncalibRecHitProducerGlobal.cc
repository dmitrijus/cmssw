...
#include "RecoLocalCalo/EcalRecProducers/plugins/EcalUncalibRecHitProducerGlobal.h"

EcalUncalibRecHitProducerGlobal::EcalUncalibRecHitProducerGlobal(
  ...
  outOfTimeThreshG12pEB_ =
      ps.getParameter<double>("outOfTimeThresholdGain12pEB");
  outOfTimeThreshG12mEB_ =
      ps.getParameter<double>("outOfTimeThresholdGain12mEB");
  outOfTimeThreshG61pEB_ =
      ps.getParameter<double>("outOfTimeThresholdGain61pEB");
  outOfTimeThreshG61mEB_ =
      ps.getParameter<double>("outOfTimeThresholdGain61mEB");
  outOfTimeThreshG12pEE_ =
      ps.getParameter<double>("outOfTimeThresholdGain12pEE");
  outOfTimeThreshG12mEE_ =
      ps.getParameter<double>("outOfTimeThresholdGain12mEE");
  outOfTimeThreshG61pEE_ =
      ps.getParameter<double>("outOfTimeThresholdGain61pEE");
  outOfTimeThreshG61mEE_ =
      ps.getParameter<double>("outOfTimeThresholdGain61mEE");
...
#ifndef USE_ES_FOR_TIMEBIAS
  // fill in the EcalTimeBiasCorrections
  ...
  timeCorrBias_ = new EcalTimeBiasCorrections();

  copy(EBtimeCorrAmplitudeBins_.begin(), EBtimeCorrAmplitudeBins_.end(),
       back_inserter(timeCorrBias_->EBTimeCorrAmplitudeBins));

  copy(EBtimeCorrShiftBins_.begin(), EBtimeCorrShiftBins_.end(),
       back_inserter(timeCorrBias_->EBTimeCorrShiftBins));

  copy(EEtimeCorrAmplitudeBins_.begin(), EEtimeCorrAmplitudeBins_.end(),
       back_inserter(timeCorrBias_->EETimeCorrAmplitudeBins));

  copy(EEtimeCorrShiftBins_.begin(), EEtimeCorrShiftBins_.end(),
       back_inserter(timeCorrBias_->EETimeCorrShiftBins));
#endif
}

EcalUncalibRecHitProducerGlobal::~EcalUncalibRecHitProducerGlobal() {
#ifndef USE_ES_FOR_TIMEBIAS
	delete timeCorrBias_;
#endif
}

void EcalUncalibRecHitProducerGlobal::set(const edm::EventSetup& es) {
  // common setup
  ...
  const EcalTimeBiasCorrectionsRcd& coRecord = es.get<EcalTimeBiasCorrectionsRcd>();
  coRecord.get(timeCorrBias_);
#endif

  //timeCorrBias_->print(std::cout);
}

// check saturation: 5 samples with gainId = 0
template <class C>
int EcalUncalibRecHitProducerGlobal::isSaturated(const C& dataFrame) {
  //bool saturated_ = 0;
  int cnt;
...
double EcalUncalibRecHitProducerGlobal::timeCorrectionEB(float ampliEB) {
  // computed initially in ns. Than turned in the BX's, as
  // EcalUncalibratedRecHit need be.
  double theCorrection = 0;

  const std::vector<float>& amplitudeBins = timeCorrBias_->EBTimeCorrAmplitudeBins;
  const std::vector<float>& shiftBins = timeCorrBias_->EBTimeCorrShiftBins;
  ...
  if (amplitudeBins.size() != shiftBins.size()) {
    ...
    edm::LogError("EcalRecHitError")
        << "Size of timeCorrAmplitudeBins different from "
           "timeCorrShiftBins. Forcing no time corrections for EB. ";
  ...
  for (int bin = 0; bin < (int) amplitudeBins.size(); bin++) {
    if (ampliEB > amplitudeBins.at(bin)) {
      myBin = bin;
    } else
      break;
  }
  ...
  if (myBin == -1) {
    ...
    theCorrection = shiftBins.at(myBin);
             ...
             myBin < ((int) amplitudeBins.size() - 1)) {
    // interpolate linearly between two assingned points
    theCorrection =
        (shiftBins.at(myBin + 1) - shiftBins.at(myBin));
    theCorrection *= (((double) ampliEB) - amplitudeBins.at(myBin)) /
                     (amplitudeBins.at(myBin + 1) -
                      amplitudeBins.at(myBin));
    theCorrection += shiftBins.at(myBin);
  } else {
    edm::LogError("EcalRecHitError")
...
double EcalUncalibRecHitProducerGlobal::timeCorrectionEE(float ampliEE) {
  // computed initially in ns. Than turned in the BX's, as
  // EcalUncalibratedRecHit need be.
  double theCorrection = 0;

  const std::vector<float>& amplitudeBins = timeCorrBias_->EETimeCorrAmplitudeBins;
  const std::vector<float>& shiftBins = timeCorrBias_->EETimeCorrShiftBins;

  if (amplitudeBins.size() != shiftBins.size()) {
	doEEtimeCorrection_ = false;
    edm::LogError("EcalRecHitError")
        << "Size of timeCorrAmplitudeBins different from "
           "timeCorrShiftBins. Forcing no time corrections for EE. ";
  ...
  int myBin = -1;
  for (int bin = 0; bin < (int) amplitudeBins.size(); bin++) {
    if (ampliEE > amplitudeBins.at(bin)) {
      myBin = bin;
    } else
      break;
  }
  ...
  if (myBin == -1) {
    ...
    theCorrection =
        (shiftBins.at(myBin + 1) - shiftBins.at(myBin));
    theCorrection *= (((double) ampliEE) - amplitudeBins.at(myBin)) /
                     (amplitudeBins.at(myBin + 1) -
                      amplitudeBins.at(myBin));
    theCorrection += shiftBins.at(myBin);
  } else {
...
bool EcalUncalibRecHitProducerGlobal::run(
  ...
  if (leadingSample >= 0) {  // saturation
    if (leadingSample != 4) {
      // all samples different from the fifth are not reliable for the amplitude
      // estimation
      // put by default the energy at the saturation threshold and flag as
      // saturated
      float sratio = 1;
      ...
      uncalibRecHit =
          EcalUncalibratedRecHit((*itdg).id(), 4095 * 12 * sratio, 0, 0, 0);
      ...
      if (detid.subdetId() == EcalEndcap) {
        ...
        // bin (or some analogous mapping) will be used instead of the
        // leadingSample
        //int bin  = (int)(( (mult * leadingSample + mult/2) * clockToNsConstant
        //+ itimeconst ) / clockToNsConstant);
        // bin is not uset for the moment
        leadingEdgeMethod_endcap_.setLeadingEdgeSample(leadingSample);
        uncalibRecHit = leadingEdgeMethod_endcap_.makeRecHit(*itdg, pedVec,
                                                             gainRatios, 0, 0);
        ...
        uncalibRecHit = leadingEdgeMethod_barrel_.makeRecHit(*itdg, pedVec,
                                                             gainRatios, 0, 0);
        uncalibRecHit.setFlagBit(EcalUncalibratedRecHit::kLeadingEdgeRecovered);
        leadingEdgeMethod_barrel_.setLeadingEdgeSample(-1);
      }
    }
    // do not propagate the default chi2 = -1 value to the calib rechit (mapped
    // to 64), set it to 0 when saturation
    ...
    if (detid.subdetId() == EcalEndcap) {
      uncalibRecHit = weightsMethod_endcap_.makeRecHit(
          *itdg, pedVec, pedRMSVec, gainRatios, weights, testbeamEEShape);
    } else {
      uncalibRecHit = weightsMethod_barrel_.makeRecHit(
    ...
    if (detid.subdetId() == EcalEndcap) {
      ratioMethod_endcap_.init(*itdg, *sampleMask_, pedVec, pedRMSVec,
                               gainRatios);
      ...
      if (doEEtimeCorrection_)
        theTimeCorrectionEE = timeCorrectionEE(uncalibRecHit.amplitude());
      uncalibRecHit.setJitter(crh.timeMax - 5 + theTimeCorrectionEE);
      uncalibRecHit.setJitterError(std::sqrt(
          pow(crh.timeError, 2) +
          std::pow(EEtimeConstantTerm_, 2) / std::pow(clockToNsConstant, 2)));
      ...
      if (uncalibRecHit.amplitude() > pedRMSVec[0] * amplitudeThreshEE_) {
        ...
        float correctedTime =
            (crh.timeMax - 5) * clockToNsConstant + itimeconst + offsetTime;
      ...
      ratioMethod_barrel_.init(*itdg, *sampleMask_, pedVec, pedRMSVec,
                               gainRatios);
      ...
      if (doEBtimeCorrection_)
        theTimeCorrectionEB = timeCorrectionEB(uncalibRecHit.amplitude());

      uncalibRecHit.setJitter(crh.timeMax - 5 + theTimeCorrectionEB);

      uncalibRecHit.setJitterError(std::sqrt(
          std::pow(crh.timeError, 2) +
          std::pow(EBtimeConstantTerm_, 2) / std::pow(clockToNsConstant, 2)));
      uncalibRecHit.setOutOfTimeEnergy(crh.amplitudeMax);
      // consider flagging as kOutOfTime only if above noise
    ...
    if (detid.subdetId() == EcalEndcap) {
      ...
      EcalUncalibRecHitRecChi2Algo<EEDataFrame> chi2expressEE_(
          *itdg, amplitude, (itimeconst + offsetTime), amplitudeOutOfTime,
          jitter, pedVec, pedRMSVec, gainRatios, testbeamEEShape,
          EEchi2Parameters_);
      ...
      EcalUncalibRecHitRecChi2Algo<EBDataFrame> chi2expressEB_(
          *itdg, amplitude, (itimeconst + offsetTime), amplitudeOutOfTime,
          jitter, pedVec, pedRMSVec, gainRatios, testbeamEBShape,
          EBchi2Parameters_);
...
DEFINE_FWK_MODULE(EcalUncalibRecHitProducerGlobal);
